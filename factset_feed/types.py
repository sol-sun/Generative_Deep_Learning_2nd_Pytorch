...
class FactSetConsolidatedFinancialRecord(BaseModel):
    """FactSet 連結財務データレコード（読み取り専用）。

    概要:
    - WolfPeriodで表される各期間について、連結ベースの売上・利益・資産・負債・税率などを保持します。
    - 金額や比率は浮動小数点数（float）で表現します。

    特徴:
    - 連結財務指標の自動計算（負債資本比率など）
    - 会計期や地域情報の付与
    - 期間管理による時系列データの整理
    - 子会社・関連会社を含む連結ベースの財務データ
    """
...
class FactSetSegmentFinancialRecord(BaseModel):
    """FactSet セグメント財務データレコード（読み取り専用）。

    概要:
    - 企業のセグメント別財務データ（売上、営業利益、資産）を保持します。
    - セグメント比率の計算結果も含みます。

    特徴:
    - セグメント別の売上・利益・資産データ
    - 各種比率の自動計算（売上比率、利益比率、資産比率）
    - 会計期間の正規化
    - セグメント別の財務指標を提供
    """
    ...
    # セグメント情報
    segment_label: constr(min_length=1, max_length=255) = Field(
        description="セグメント名",
        examples=["iPhone", "Services", "Mac"]
    )
    segment_label_normalized: Optional[constr(min_length=1, max_length=255)] = Field(
        default=None,
        description="正規化されたセグメント名",
        examples=["iphone", "services", "mac"]
    )
    segment_number: Optional[conint(ge=1, le=99)] = Field(
        default=None,
        description="セグメント番号",
        examples=[1, 2, 3]
    )
    ...
    # セグメント財務データ（float型）
    sales_external: Optional[float] = Field(
        default=None,
        description="外部売上高",
        examples=[123456000000.0]
    )
    sales_internal: Optional[float] = Field(
        default=None,
        description="内部売上高",
        examples=[5000000000.0]
    )
    sales_total: Optional[float] = Field(
        default=None,
        description="総売上高（外部+内部）",
        examples=[128456000000.0]
    )
    operating_income: Optional[float] = Field(
        default=None,
        description="営業利益",
        examples=[45678000000.0]
    )
    assets: Optional[float] = Field(
        default=None,
        description="資産",
        examples=[789012000000.0]
    )
    
    # セグメント比率（小数点形式）
    sales_ratio: Optional[float] = Field(
        default=None,
        description="売上比率（総売上に対する割合）",
        examples=[0.3512]
    )
    sales_external_ratio: Optional[float] = Field(
        default=None,
        description="外部売上比率（外部売上合計に対する割合）",
        examples=[0.3378]
    )
    operating_income_ratio: Optional[float] = Field(
        default=None,
        description="営業利益比率（営業利益合計に対する割合）",
        examples=[0.3995]
    )
    assets_ratio: Optional[float] = Field(
        default=None,
        description="資産比率（資産合計に対する割合）",
        examples=[0.2234]
    )
    
    # 調整項目
    reconciling_sales: Optional[float] = Field(
        default=None,
        description="調整項目売上高",
    ...
    fiscal_year: Optional[conint(ge=1900, le=2999)] = Field(
        default=None,
        description="会計年度",
        examples=[2023]
    )
    ...
    @computed_field
    @property
    def is_reconciling_item(self) -> bool:
        """調整項目かどうかの判定。"""
        return self.segment_label.lower() == "reconciling items"
