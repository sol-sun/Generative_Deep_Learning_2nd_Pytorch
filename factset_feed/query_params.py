...
class FactSetSegmentQueryParams(BaseModel):
    """FactSet セグメントクエリパラメータ。

    目的:
    - セグメントデータ取得の条件を安全に指定
    - セグメント別フィルタリングとデータ品質制御
    """
    ...
    # セグメントフィルタ
    segment_labels: Optional[List[constr(min_length=1, max_length=255)]] = Field(
        default=None,
        description="特定のセグメント名リストでフィルタ",
        examples=[["iPhone", "Services", "Mac"]]
    )
    exclude_reconciling_items: bool = Field(
        default=True,
        description="調整項目を除外"
    )
    ...
    min_sales_ratio: Optional[float] = Field(
        default=None,
        description="最小売上比率（0.0-1.0）",
        examples=[0.01]
    )
    min_sales_amount: Optional[float] = Field(
        default=None,
        description="最小売上金額",
        examples=[1000000.0]
    )
    ...
    @field_validator("min_sales_ratio")
    @classmethod
    def _validate_sales_ratio(cls, v: Optional[float]) -> Optional[float]:
        """売上比率の範囲チェック。"""
        if v is not None and (v < 0.0 or v > 1.0):
            raise ValueError("売上比率は0.0から1.0の範囲で指定してください")
        return v
