...
使用例:
    from data_providers.core import DataProcessor
    
    # 基底クラスとして継承
    class MyProvider(DataProcessor):
        def __init__(self):
            super().__init__()  # データベース接続を初期化
            # 独自の処理を追加
    
    # スタティックメソッドとして直接使用
    normalized_text = DataProcessor.normalize_text("Sample Text")
    
    # 会計期間調整
    df_adjusted = DataProcessor.adjust_fiscal_term(df, date_col="DATE")
    
    # 重複期間除去
    df_clean = DataProcessor.remove_duplicate_periods(df)
...
import pandas as pd
import unicodedata
from data_providers.database import GibDB


class DataProcessor(GibDB):
    """
    データプロバイダー基底クラス
    ...
    継承方法:
        class MyDataProvider(DataProcessor):
            def __init__(self):
                super().__init__()  # データベース接続を初期化
                # 独自の処理を追加
