...
class BetaCalculator:
    ...
    def calculate_from_returns(
        ...
        *,
        ...
        return BetaResult(
            ...
            standard_error=beta_stats['standard_error'],
            levered=levered
        )
    ...
    def _calculate_beta_statistics(security_returns: pd.Series, index_returns: pd.Series) -> Dict[str, float]:
        ...
        if n > 2 and not np.isnan(beta) and r_squared > 0:
            # 残差の分散を使った標準誤差の計算
            security_var = np.var(security_returns)
            standard_error = np.sqrt((security_var * (1 - r_squared)) / ((n - 2) * index_variance))
        else:
