...
class FactSetDataManager:
    ...
    def initialize_data(self) -> Dict[str, pd.DataFrame]:
        ...
        financial_data = BaseProvider.remove_duplicate_periods(
            financial_data, 
            group_cols=["FSYM_ID", "FACTSET_ENTITY_ID"]
        )
        
        segment_data = BaseProvider.remove_duplicate_periods(
            segment_data,
    ...
    def _process_revere_segments(self, revere_data: pd.DataFrame) -> pd.DataFrame:
        ...
        df['SEGMENT_NAME_3'] = df['SEGMENT_NAME_3'].apply(BaseProvider.normalize_text)
    ...
    def _get_financial_data(self) -> pd.DataFrame:
        ...
        # 会計期間の調整
        df = BaseProvider.adjust_fiscal_term(df)
