...
- 自動更新は行いません。明示的に `CompanyCatalog.load()` を呼び出してください。
...
使用例
    ...
    catalog.load()  # DB から最新をロード
    # または、フィルタを指定:
    # catalog.load(active_only=True, primary_equity_only=True, listed_only=True)
...
from gppm.utils.country_code_manager import convert_to_alpha2, Alpha2Code, Alpha3Code, normalize_country_code
from wolf_period import WolfPeriod
from data_providers.data_sources.factset.provider import FactSetProvider
from data_providers.data_sources.factset.query_params import FactSetQueryParams
from data_providers.data_sources.factset.types import FactSetIdentityRecord
from data_providers.data_sources.ciq.provider import CIQProvider
from data_providers.data_sources.ciq.query_params import CIQQueryParams
from data_providers.data_sources.ciq.types import CIQIdentityRecord
...
class Company(BaseModel):
    ...
    # 地理（上場国/本社国）。ISO 3166-1 alpha-2/alpha-3 両対応
    ...
    @field_validator("exchange_country_code", "headquarters_country_code", mode="before")
    @classmethod
...
class CompanyCatalog(BaseProvider):
    ...
    def __init__(self, max_workers: int = 4, batch_size: int = 5000) -> None:
        ...
        self._lock = Lock()
        self._last_load: Optional[WolfPeriod] = None  # WolfPeriod使用
    ...
    def load(
        ...
        *,
        ...
        primary_equity_only: bool = False,
        listed_only: bool = True
    ) -> None:
        """データをロードします（高速・WolfPeriod対応）。

        Args:
            active_only: アクティブな証券のみを取得するかどうか
            primary_equity_only: 主力証券のみを取得するかどうか
            listed_only: 上場証券のみを取得するかどうか
        ...
        load_period = WolfPeriod.from_day(datetime.now(timezone.utc).date())
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            fs_future = executor.submit(
                ...
                FactSetQueryParams(
                    ...
                    primary_equity_only=primary_equity_only,
        ...
        fs_errors, ciq_errors = self._convert_records_batch(
            fs_records, ciq_records, companies, load_period
        )
        ...
        t4 = perf_counter()
        self._last_load = load_period  # WolfPeriod使用
    ...
    def search(
        ...
        *,
        ...
        for c in self._by_pk.values():
            if s.country_code:
                cc = normalize_country_code(s.country_code)
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_load_period": self._last_load.model_dump() if self._last_load else None,
            "batch_size": self._batch_size,
            "providers": {
