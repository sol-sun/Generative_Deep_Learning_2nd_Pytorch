...
class RBICSProvider(BaseProvider):
            ...
            for _, row in batch_df.iterrows():
                try:
                    ...
                    record = RBICSCompanyRecord(
                        ...
                        period_end_date=row.get("PERIOD_END_DATE") if row.get("PERIOD_END_DATE") is not None else None,
                        retrieved_period=retrieved_period,
                    )
                    batch_records.append(record)
                except Exception as e:
                    validation_errors += 1
                    logger.debug(
                        "RBICS企業レコード検証エラー: factset_entity_id=%s error=%s",
